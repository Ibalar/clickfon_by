...
class CompareView extends View
{
	...
	{
        $isAjax = $this->request->get('ajax', 'integer') == 1;
		// Add Product
        if ($this->request->get('product_url', 'string')) {
            ...
            if ($isAjax) {
                ...
                echo json_encode([
                    'success' => true,
                    'compare_count' => count($_SESSION['compared_products'])
                ]);
                exit;
            } else {
        ...
        if ($this->request->get('remove_product_url', 'string')) {
            ...
            if ($isAjax) {
                ...
                echo json_encode([
                    'success' => true,
                    'compare_count' => isset($_SESSION['compared_products']) ? count($_SESSION['compared_products']) : 0
                ]);
                exit;
            } else {
                header('location: ' . $this->config->root_url . '/' . $this->langLink . 'compare/');
