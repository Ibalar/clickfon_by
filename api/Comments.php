...
class Comments extends Turbo
{
    ...
    {
        // Get old comment data to check if we need to update product stats
        $this->db->query("SELECT object_id, type, approved FROM __comments WHERE id=?", (int) $id);
        $oldComment = $this->db->result();
    ...
    /**
     * Update denormalized comment statistics for a product
     * This method recalculates avg_rating and total_comments for a specific product
     */
    private function updateProductCommentStats($productId)
    {
        if (!$productId) {
            return;
        }
        ...
        $this->db->query("
            ...
            WHERE id = ?
        ", (int) $productId, (int) $productId, (int) $productId);
    }
}
