-- Migration: Add denormalized fields to t_products table for variants optimization
...
CREATE INDEX idx_products_has_stock_position ON t_products(has_stock DESC, position DESC);
CREATE INDEX idx_products_has_stock_min_price ON t_products(has_stock DESC, min_price ASC);
CREATE INDEX idx_products_has_stock_name ON t_products(has_stock DESC, name ASC);
CREATE INDEX idx_products_has_stock_rating ON t_products(has_stock DESC, rating DESC, position DESC);
CREATE INDEX idx_products_has_stock_avg_rating ON t_products(has_stock DESC, avg_rating DESC, position DESC);
CREATE INDEX idx_products_min_price ON t_products(min_price);

-- Add indexes on variants table for better JOIN performance
CREATE INDEX idx_variants_product_stock_position ON t_variants(product_id, stock, position);
CREATE INDEX idx_variants_product_price ON t_variants(product_id, price);
